    ...
    with plt.style.context(('ggplot')):
        for campaign in tqdm(campaigns):
            ...
            for name in o.NAMES:
                with silence():
                    try:
                        df = K2ephem.get_ephemeris_dataframe(name[0], campaign, campaign, step_size=1/8)
                ...
                if np.isfinite(np.nanmedian(ra) * np.nanmedian(dec)):
                    ax.text(np.nanmedian(ra), np.nanmedian(dec)+0.2, name[0], color=c, zorder=99, fontsize=10)
